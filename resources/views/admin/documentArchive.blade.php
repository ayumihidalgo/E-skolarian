                            ...
                            <tbody>
                                @for ($i = 0; $i < 20; $i++)
                                    ...
                                    <tr class="border-b border-gray-300 hover:bg-gray-100 cursor-pointer"
                                        ...
                                        data-org-acronym="{{ $acronym }}" data-status="{{ $status }}"
                                        data-type="{{ $docType }}">
                                        <td class="px-4 py-2 font-semibold truncate max-w-[120px]"><span
                                                class="{{ $tagColor }}">{{ $tag }}</span></td>
                                        <td class="px-4 py-2 truncate max-w-[160px]" title="{{ $orgName }}">
                                            {{ $orgName }}</td>
                                        <td class="px-4 py-2 truncate max-w-[160px]" title="Document Title">
                                            {{ $acronym }}_Project_{{ $i + 1 }}</td>
                                        <td class="px-4 py-2 truncate max-w-[120px]">
                                            {{ now()->subDays($i)->format('m/d/Y') }}
                                        </td>
                                        <td class="px-4 py-2 truncate max-w-[160px]" title="{{ $docType }}">
                                            {{ $docType }}</td>
                                        <td class="px-4 py-2">
                                            <span
                                                class="px-4 py-1 rounded-full text-white inline-block min-w-[100px] text-center {{ $status === 'Approved' ? 'bg-[#10B981]' : 'bg-[#EF4444]' }}">{{ $status }}</span>
                                        </td>
                                    </tr>
                                @endfor
                            </tbody>
            ...
            <div class="mt-4 flex justify-center">
                <nav>
                    <ul class="inline-flex items-center space-x-2">
                        <li><button class="pagination-btn-first px-3 py-1 rounded-lg"
                                onclick="setActivePage(this)">First</button></li>
                        <li><button class="pagination-btn px-3 py-1 rounded-lg bg-[#7A1212] text-white"
                                onclick="setActivePage(this)">1</button></li>
                        <li><button class="pagination-btn px-3 py-1 rounded-lg" onclick="setActivePage(this)">2</button>
                        </li>
                        <li><button class="pagination-btn px-3 py-1 rounded-lg" onclick="setActivePage(this)">3</button>
                        </li>
                        <li><button class="pagination-btn-last px-3 py-1 rounded-lg"
                                onclick="setActivePage(this)">Last</button></li>
                    </ul>
                </nav>
            </div>
        </div>
            ...
            function applyFilters() {
                ...
                rows.forEach(row => {
                    ...
                    const matchesStatus = statusFilter === "Status" || statusFilter === "All" || status ===
                        statusFilter;
                    const matchesOrganization = organizationFilter === "Organization" || organizationFilter === "All" ||
