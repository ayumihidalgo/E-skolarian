...
<div id="sidebar"
    ...
    <nav class="flex flex-col justify-between h-full mt-6">
        <div class="space-y-6 text-lg font-[Manrope]">
            @foreach ([['Dashboard', 'newDashboard.svg', route('admin.dashboard')], ['Review', 'review.svg', route('admin.documentReview')], ['Repository', 'archive.svg', route('admin.documentHistory')], ['Calendar', 'calendar.svg', route('calendar.indexTwo')], ['Settings', 'settings.svg', route('admin.settings')]] as [$label, $icon, $route])
                <a href="{{ $route }}"
