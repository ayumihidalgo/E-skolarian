...
<div id="main-content" class="flex flex-col min-h-screen bg-[#F2F4F7] font-['Lexend']">
    ...
    <div class="flex-grow flex items-center justify-center p-4">
        <div class="flex flex-col justify-center h-full space-y-4 w-full max-w-[1000px]">
            @include('components.guestProgress', ['step' => 3])
            <div class="space-y-2">
                <h2 class="text-2xl font-semibold">Submission Form</h2>
                <p>Please fill up the following fields:</p>
            </div>

            <form class="font-['Manrope'] w-full" action="{{ route('submit.document') }}" method="POST"
...
<script>
    ...
    const guestShouldRedirect = "{{ session('guest_confirm_popup') ? 'true' : 'false' }}" === 'true';

    function closeControlTagPopup() {
        document.getElementById("controlTagPopup").classList.add("hidden");

        if (guestShouldRedirect) {
            // Redirect to success page only when Confirm is clicked
            setTimeout(() => {
                window.location.href = "{{ route('guest.submissionSuccess') }}";
            }, 200);
        }
    }
