...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let addUserFormHasChanges = false;
    const addUserBtn = document.getElementById("addUserBtn");
    const addUserModal = document.getElementById("addUserModal");
    const closeAddUserBtn = document.getElementById("closeAddUserBtn");
    const closeAddUserModalBtn = document.getElementById(
        "closeAddUserModalBtn"
    );
    ...
    const backToRoleFromAdminBtn = document.getElementById(
        "backToRoleFromAdminBtn"
    );
    ...
    const customRoleContainer = document.getElementById(
    ...
    const customRoleTypeRadios = document.querySelectorAll(
        'input[name="custom_role_type"]'
    );

    // Student form elements
    const organizationNameInput = document.getElementById("organization_name");
    const organizationAcronymInput = document.getElementById(
    ...
    function checkAddUserFormDirty() {
        if (!addUserForm) return false;
        // Check all input, select, textarea fields
        return Array.from(
            addUserForm.querySelectorAll("input, select, textarea")
        ).some((el) =>
    ...
    if (addUserForm) {
        addUserForm.addEventListener("input", function () {
            addUserFormHasChanges = checkAddUserFormDirty();
        });
        addUserForm.addEventListener("change", function () {
            addUserFormHasChanges = checkAddUserFormDirty();
        });
    }
    if (addUserModal) {
        // When modal is opened, reset the flag
        addUserModal.addEventListener("transitionend", function () {
    ...
    window.addEventListener("beforeunload", function (e) {
        // Only trigger if modal is open and form has changes
        if (
            addUserModal &&
            !addUserModal.classList.contains("hidden") &&
            addUserFormHasChanges
        ) {
            e.preventDefault();
            e.returnValue =
                "You have unsaved changes in the Add User form. Are you sure you want to leave?";
    ...
    if (adminEmailInput) {
        adminEmailInput.parentNode.insertBefore(
            adminEmailError,
            adminEmailInput.nextSibling
        );
    }
    if (studentEmailInput) {
        studentEmailInput.parentNode.insertBefore(
            studentEmailError,
            studentEmailInput.nextSibling
        );
    }
    if (organizationNameInput) {
        organizationNameInput.parentNode.insertBefore(
            organizationNameError,
            organizationNameInput.nextSibling
        );
    }
    if (organizationAcronymInput) {
        organizationAcronymInput.parentNode.insertBefore(
            organizationAcronymError,
            organizationAcronymInput.nextSibling
        );
    }
    if (roleSelect) {
    ...
    if (customRoleName) {
        customRoleName.parentNode.insertBefore(
            customRoleNameError,
            customRoleName.nextSibling
        );
    }

    // Insert role type error after custom role type container
    const customRoleTypeContainer = document
        .querySelector('input[name="custom_role_type"]')
        ?.closest(".space-y-2");
            ...
            if (roleSelect.value === "custom_role") {
                ...
                if (checkedRadio) {
                    ...
                    if (roleType === "student") {
                        ...
                        const existingOrgFields = document.getElementById(
                            "existing-org-fields"
                        );
                        const newOrgFields =
                            document.getElementById("new-org-fields");
                        ...
                        if (organizationAcronymInput) {
                            ...
                            organizationAcronymInput.classList.remove(
                                "bg-gray-100",
                                "cursor-not-allowed"
                            );
                        }
        ...
        roleSelect.addEventListener("change", function () {
            ...
            if (this.value === "custom_role") {
                ...
                const customRoleName =
                    document.getElementById("custom_role_name");
                if (customRoleName) {
                    customRoleName.value = "";
                }

                document
                    .querySelectorAll('input[name="custom_role_type"]')
                    .forEach((radio) => {
                        radio.checked = false;
                    });
    ...
    const customOrgTypeRadios = document.querySelectorAll(
        'input[name="custom_org_type"]'
    );
    ...
    if (organizationNameInput) {
        organizationNameInput.addEventListener("change", function () {
            ...
            if (selectedOrganization && organizationAcronymInput) {
                ...
                const selectedOption =
                    organizationNameInput.options[
                        organizationNameInput.selectedIndex
                    ];
                ...
                if (acronymMatch && acronymMatch[1]) {
                    ...
                    organizationAcronymInput.classList.add(
                        "bg-gray-100",
                        "cursor-not-allowed"
                    );
                }
        ...
        const nonAcademicOrgBtn = document.getElementById(
            "non-academic-org-btn"
        );
        ...
        if (academicOrgBtn && nonAcademicOrgBtn) {
            ...
            nonAcademicOrgBtn.addEventListener("click", function () {
                ...
                nonAcademicOrgBtn.classList.remove(
                    "bg-gray-200",
                    "text-gray-700"
                );
    ...
    function updateContinueButton() {
        ...
        if (roleSelect.value === "custom_role") {
            ...
            continueToNextBtn.classList.toggle(
                "cursor-not-allowed",
                !roleValid
            );
        }
    ...
    function updateStudentSubmitButton() {
        ...
        if (
            isNewRole &&
            newOrgFields &&
            !newOrgFields.classList.contains("hidden")
        ) {
            organizationField = document.getElementById(
                "new_organization_name"
            );
        } else if (
            existingOrgFields &&
            !existingOrgFields.classList.contains("hidden")
        ) {
    ...
    const newOrganizationNameInput = document.getElementById(
        "new_organization_name"
    );
    ...
    function updateRoleOptions(existingRoles) {
        ...
        options.forEach((option) => {
            ...
            const isRestricted =
                restrictedRoles.includes(roleName) &&
    ...
    async function updateOrganizationDropdown(orgType) {
        ...
        try {
            ...
            const academicOrgs = [
                {
                    name: "Association of Competent and Aspiring Psychologists",
                    acronym: "ACAP",
                },
                {
                    name: "Association of Electronics and Communications Engineering Students",
                    acronym: "AECES",
                },
                {
                    name: "Eligible League of Information Technology Enthusiast",
                    acronym: "ELITE",
                },
                {
                    name: "Guild of Imporous and Valuable Educators",
                    acronym: "GIVE",
                },
                {
                    name: "Junior Executives of Human Resources Association",
                    acronym: "JEHRA",
                },
                {
                    name: "Junior Marketing Association of the Philippines",
                    acronym: "JMAP",
                },
                {
            ...
            const nonAcademicOrgs = [
                ...
                {
                    name: "Supreme Innovators' Guild for Mathematics Advancement",
                    acronym: "SIGMA",
                },
                {
                    name: "Transformation Advocates through Purpose-driven and Noble Objectives Toward Community Holism",
                    acronym: "TAPNOTCH",
                },
            ];
            ...
            let orgsToShow =
                orgType === "academic" ? academicOrgs : nonAcademicOrgs;
    ...
    async function validateCustomRole() {
        ...
        // Check for leading invalid characters
            ...
            showCustomRoleNameError(
                "Role name cannot start with an apostrophe"
            );
            isValid = false;
        }
        ...
        // Check minimum length
            ...
            showCustomRoleNameError(
                `Role name must be at least ${MIN_ROLE_LENGTH} characters long`
            );
            isValid = false;
        }
        // Check maximum length
            ...
            showCustomRoleNameError(
                `Role name must be less than ${MAX_ROLE_LENGTH} characters`
            );
            isValid = false;
        }
        // Check allowed characters (only letters, spaces, hyphens, and apostrophes)
            ...
            showCustomRoleNameError(
                "Role name can only contain letters, spaces, hyphens, and apostrophes"
            );
            isValid = false;
        }
        // Check for multiple consecutive spaces
            ...
            showCustomRoleNameError(
                "Role name cannot contain multiple consecutive spaces"
            );
            isValid = false;
        }
        // Check for multiple consecutive hyphens or apostrophes
            ...
            showCustomRoleNameError(
                "Role name cannot contain multiple consecutive hyphens or apostrophes"
            );
            isValid = false;
        } else {
            ...
            const roleExists = existingAdminRoles.some(
                (role) => role.toLowerCase() === customRole.toLowerCase()
            );

            if (roleExists) {
                showCustomRoleNameError(
                    "This role name already exists as an administrative role"
                );
                isValid = false;
            } else {
                ...
                try {
                    ...
                    const isDuplicate = existingRoles.some(
                        (role) =>
                            ...
                            customRole.toLowerCase().trim()
                    );

                    if (isDuplicate) {
                        showCustomRoleNameError(
                            "This role name already exists in the system"
                        );
                        isValid = false;
                    }
                } catch (error) {
                    ...
                    showCustomRoleNameError(
                        "Error checking role availability. Please try again."
                    );
                    isValid = false;
                }
            }
        ...
        if (!roleTypeRadio) {
            roleTypeError.textContent =
                "Please select whether this is a Student Organization or Campus Administration role";
    ...
    async function validateOrganizationName() {
        ...
        if (exists) {
            showOrganizationNameError(
                "This organization already exists in the system"
            );
            return false;
        }
    ...
    async function validateOrganizationAcronym() {
        ...
        if (acronym === "") {
            showOrganizationAcronymError(
                "Organization acronym cannot be empty"
            );
            return false;
        }
        ...
        if (!/^[A-Z]+$/.test(acronym)) {
            showOrganizationAcronymError(
                "Acronym can only contain uppercase letters"
            );
            return false;
        }
    ...
    async function validateStudentEmail() {
        ...
        if (!hasValidDomain) {
            showStudentEmailError(
                "Please use a valid @gmail.com, @yahoo.com, @iskolarngbayan.pup.edu.ph, or @pup.edu.ph email address"
        ...
        submitStudentBtn.addEventListener("click", async function (e) {
            ...
            try {
                ...
                const organizationName = isNewRole
                    ? document
                          .getElementById("new_organization_name")
                          .value.trim()
                    : organizationNameInput.value.trim();
        ...
        if (successModal) {
            ...
            if (successMessage) {
                successMessage.textContent =
                    "The user account has been added successfully.";
            }
            ...
            const okayButton = document.querySelector(
                '#successModal button[type="button"]'
            );
            const closeSuccessBtn = document.querySelector(
                "#successModal #closeSuccessModalBtn"
            );
    ...
    function handleSubmissionError(error) {
        ...
        const formError =
            ...
            document.getElementById("adminFormError");
        if (formError) {
    ...
    function disableFormButtons() {
        ...
        if (backToRoleFromAdminBtn) {
            ...
            backToRoleFromAdminBtn.classList.add(
                "opacity-50",
                "cursor-not-allowed"
            );
        }
    ...
    function enableFormButtons() {
        ...
        if (submitStudentBtn) {
            submitStudentBtn.disabled = false;
            submitStudentBtn.innerHTML = "Add User";
            submitStudentBtn.classList.remove(
                "opacity-50",
                "cursor-not-allowed"
            );
        }
        ...
        if (backToRoleFromAdminBtn) {
            ...
            backToRoleFromAdminBtn.classList.remove(
                "opacity-50",
                "cursor-not-allowed"
            );
        }
