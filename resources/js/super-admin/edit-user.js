...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const closeEditConfirmModal = document.getElementById(
    ...
    let editUserFormHasChanges = false;
    function checkEditUserFormDirty() {
        if (!editUserForm) return false;
        return Array.from(
            editUserForm.querySelectorAll("input, select, textarea")
        ).some((el) =>
    ...
    if (editUserForm) {
        editUserForm.addEventListener("input", function () {
            editUserFormHasChanges = checkEditUserFormDirty();
        });
        editUserForm.addEventListener("change", function () {
            editUserFormHasChanges = checkEditUserFormDirty();
        });
    }

    // Listen for modal open/close to reset flag
    if (editUserModal) {
        editUserModal.addEventListener("transitionend", function () {
    ...
    window.addEventListener("beforeunload", function (e) {
        if (
            ...
            editUserFormHasChanges
        ) {
            e.preventDefault();
            e.returnValue =
                "You have unsaved changes in the Edit User form. Are you sure you want to leave?";
            return e.returnValue;
        }
    });

    // Intercept modal close button
    ...
    if (recoveryEmailInput) {
        // Add this block
        recoveryEmailInput.parentNode.insertBefore(
            recoveryEmailError,
            recoveryEmailInput.nextSibling
        );
    }
    if (roleSelect) {
    ...
    function resetEditForm() {
        if (editUserForm) {
            ...
            document.getElementById("editRecoveryEmail").value =
                initialFormState.recoveryEmail || ""; // Add this line
            document.getElementById("editRoleName").value =
                initialFormState.roleName || "";
    ...
    function captureInitialState() {
        initialFormState = {
            ...
            recoveryEmail: document
                .getElementById("editRecoveryEmail")
                .value.trim(), // Add this line
    ...
    async function validateRecoveryEmail() {
        ...
        if (
            ...
            initialFormState.recoveryEmail.toLowerCase()
        ) {
            try {
    ...
    if (editUserForm) {
        editUserForm.addEventListener("submit", async function (e) {
            ...
            try {
                ...
                formData.append(
                    "recovery_email",
                    document.getElementById("editRecoveryEmail").value.trim()
                ); // Add this line
                formData.append(
                    "username",
                    document.getElementById("editUsername").value
                ...
                if (
                    !isAdmin &&
                    editAcronymField &&
                    !editAcronymField.classList.contains("hidden")
                ) {
                    formData.append(
                        "organization_acronym",
                        document.getElementById("editAcronym").value.trim()
                    );
                }
